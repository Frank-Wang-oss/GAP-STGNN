...
class Feature_extractor_1DCNN_REG(nn.Module):
    def __init__(self, input_channels, num_hidden, out_dim, kernel_size = 8, stride = 1, dropout = 0):
        super(Feature_extractor_1DCNN_REG, self).__init__()
...
class MPNN_block_seperate(nn.Module):
    def __init__(self, input_dim, output_dim, num_sensors, time_conv, time_window_size, stride, decay, pool_choice):
        ...
        self.Temporal = Feature_extractor_1DCNN_REG(input_dim, input_dim*2, input_dim*2,kernel_size=3)
...
class BinaryGaussianPatchLayer_v3(nn.Module):
    def __init__(self, max_windows, input_length, num_channels):
        super().__init__()
        self.num_win = max_windows
        self.input_length = input_length
        self.ch = num_channels

        # Learnable parameter networks
        self.channel_proj = nn.Conv1d(num_channels, num_channels, 3, padding=1)

        # Temporal attention for parameter generation
        self.temp_attn = nn.Sequential(
            nn.Conv1d(num_channels, 16, 5, padding=2),
            nn.ReLU(),
            nn.AdaptiveAvgPool1d(1),
            nn.Flatten(),
            nn.Linear(16, max_windows * 2)
        )
        with tr.no_grad():
            self.temp_attn[-1].bias[:max_windows] = tr.linspace(-2, 2, max_windows)
        # Gated parameter modulation

        # Threshold parameters
        self.threshold = nn.Parameter(tr.tensor(0.3))
        self.register_buffer('pos', tr.linspace(0, 1, input_length))

    def _compute_gaussian(self, mu, sigma):
        """Gaussian weights calculation"""
        # pos: [L], mu: [B, W], sigma: [B, W]
        return tr.exp(-0.5 * ((self.pos[None, None, :] - mu[:, :, None]) /
                              sigma[:, :, None].clamp(min=1e-4))  **  2)

    def forward(self, x):
        B = x.size(0)

        # 1. Feature projection
        x_proj = self.channel_proj(x)  # [B, ch, L]

        # 2. Learn parameters from input
        # Temporal attention
        attn_feat = self.temp_attn(x_proj)  # [B, 2*W]
        mu_raw, sigma_raw = attn_feat.chunk(2, dim=1)  # [B, W] each

        # Gated modulation
        mu = mu_raw.sigmoid()  # [B, W] ∈ (0,1)
        # print(mu)
        sigma =sigma_raw.exp()  # [B, W] >0
        sigma = 0.1+0.1 * sigma
        # print(sigma[0])

        # 3. Compute Gaussian masks
        weights = self._compute_gaussian(mu, sigma)  # [B, W, L]


        # 4. Adaptive thresholding with gradient flow
        hard_mask = (weights > self.threshold).float()  # [B, W, L]

        # Backforward：sigmoid gradient approximate
        sigmoid_mask = tr.sigmoid(100 * (weights - self.threshold))
        soft_mask = sigmoid_mask + (hard_mask - sigmoid_mask).detach()  # STE

        # soft_mask = soft_mask.permute(1, 0)  # [B, W, L]
        kernel = soft_mask.view(B, self.num_win, 1, self.input_length)
        x = x.unsqueeze(1)
        x_patch = x * kernel  # [batch, W, ch, L]

        return x_patch  # [B, W, ch, L]



class AdaptivePatchLayer_v4(nn.Module):
    def __init__(self, input_length, num_channels, d=8):
        super().__init__()
        self.ch = num_channels
        self.L = input_length

        self.feature_extractor = nn.Sequential(
            nn.Conv2d(1, d, kernel_size=(3, 5), padding=(1, 2)),
            nn.ReLU(),
            # nn.LayerNorm([d, num_channels, input_length]),
        ...
        self.query = nn.Parameter(tr.randn(d * 2) * 0.1)
        self.key_proj = nn.Linear(d * 2, d * 2)

        # Threshold generator with constrained range
        ...
        # Decay controller with temperature scaling
        self.decay_generator = nn.Sequential(
            nn.Linear(d * 2, 1),
            nn.Sigmoid()
        )

        # Learnable saliency scaler
        self.saliency_scale = nn.Parameter(tr.tensor(1.0))

        # Learnable threshold base
        self.threshold_base = nn.Parameter(tr.tensor(0.0))
    ...
    def forward(self, x):
        ...
        # print(x.size())
        x_4d = x.reshape(B * W, 1, C, L)
        features = self.feature_extractor(x_4d).reshape(B, W, -1)

        # Compute saliency with scaled magnitude
        keys = self.key_proj(features)
        saliency = tr.einsum('bwd,d->bw', keys, self.query) * self.saliency_scale

        # Generate adaptive parameters
        global_features = features.mean(dim=1)

        # Threshold in [base - 1, base + 1] range
        threshold = self.threshold_base + self.threshold_generator(global_features).squeeze(1)

        # Controlled decay weights
        decay = self.decay_generator(global_features).squeeze(1)

        # Stabilized comparison
        scaled_saliency = saliency - saliency.mean(dim=1, keepdim=True)
        above_threshold = (scaled_saliency > threshold.unsqueeze(1)).float()

        window_weights = above_threshold + (1 - above_threshold) * decay.unsqueeze(1)
        # print(window_weights)
        return x * window_weights.unsqueeze(-1).unsqueeze(-1)

def calculate_output_length(input_length, model):
    with tr.no_grad():
        dummy_input = tr.randn(1, input_length, model.conv_block1[0].in_channels)
