...
class CMPDataIter(data.IterableDataset):
    ...
    def __init__(self, data_root, data_set, max_rul, seq_len, net_name):
        ...
        logging.info("CMPDataIter:: iterator initialized (train UEA_data_new shape: {:})".format(len(self.train_x)))
        ...
        logging.info("CMPDataIter:: iterator initialized (test UEA_data_new shape: {:})".format(len(self.test_x)))
        ...
        # for i in range(len(self.folded_train_x)):
        #     print(np.shape(self.folded_train_x[i]))
    ...
    def _get_data(self, data_root, data_set):
        ...
        assert os.path.exists(train_data_pt), 'UEA_data_new path does not exist: {:}'.format(train_data_pt)
        # print(train_data_pt)
        test_data_pt = os.path.join(data_root, 'CMAPSSData', 'test_'+ data_set +'.txt')
        assert os.path.exists(test_data_pt), 'UEA_data_new path does not exist: {:}'.format(test_data_pt)

        test_truth_pt = os.path.join(data_root, 'CMAPSSData', 'RUL_'+ data_set +'.txt')
        assert os.path.exists(test_truth_pt), 'UEA_data_new path does not exist: {:}'.format(test_truth_pt)
    ...
    def _process(self, train_df, test_df, test_truth):
        ...
        # normailize both train and test UEA_data_new
        ...
        if condition_num > 1:
            logging.info("CMPDataIter:: UEA_data_new includes multi operating conditions")
        else:
            logging.info("CMPDataIter:: UEA_data_new includes single operating conditions")
        
        # generate final UEA_data_new:
        ...
        for i in range(train_engine_num):
            ...
            if end_index - start_index < self.seq_len-1:
                print('train UEA_data_new less than seq_len!')
            # for sensor train matrix, number of 21 X 15 needed per UEA_data_new points (minus the first sequence length) per engine, so the array input start from start index
        ...
        for i in range(train_engine_num):
            end_index = start_index+train_rul.loc[i, 'max']
            # print(end_index)
            # for ops train matrix, number of 3 X 15 needed per UEA_data_new points (minus the first sequence length) per engine, so the array input start from start index
            #settings UEA_data_new are in the first 3 columns of Train_Norm
        ...
        for i in range(test_engine_num):
            ...
            if end_index - start_index < self.seq_len:
                print('Sensor::test UEA_data_new ({:}) less than seq_len ({:})!'
                    .format(end_index - start_index, self.seq_len))

                # simply pad the first UEA_data_new serveral times:
                print('Sensor::Use first UEA_data_new to pad!')
        ...
        for i in range(test_engine_num):
            ...
            if end_index - start_index < self.seq_len:
                print('Setting::test UEA_data_new ({:}) less than seq_len ({:})!'
                    .format(end_index - start_index, self.seq_len))

                # simply pad the first UEA_data_new serveral times:
                print('Setting::Use first UEA_data_new to pad!')
                num_pad = self.seq_len - (end_index - start_index)
